...
function getCachedFormatter(
  ...
  const { currency, maximumFractionDigits = 2, minimumFractionDigits = 0 } = opts;
...
function applyMask(opts: {
  ...
  const { currency, locale, mask, pattern, value } = opts;
...
function applyCurrencyMask(opts: { value: string; currency?: string; locale?: string }): string {
  const { currency = DEFAULT_CURRENCY, locale = DEFAULT_LOCALE, value } = opts;
  ...
  const {
    ...
    decimal: decimalSeparator,
    group: groupSeparator,
  } = getCachedCurrencySymbols({ currency, locale });
...
function getUnmaskedValue(opts: {
  ...
  const { currency, locale, transform, value } = opts;
...
function toUnmaskedIndex(opts: { masked: string; pattern: string; caret: number }): number {
  const { caret, masked, pattern } = opts;
...
function getCurrencyCaretPosition(opts: {
  ...
  const { mask, newValue, transformOpts } = opts;
...
function getPatternCaretPosition(opts: {
  ...
  const { currentUnmasked, maskPattern, newValue } = opts;
  let position = 0;
  let unmaskedCount = 0;
...
const MaskInput = (props: Readonly<MaskInputProps>): JSX.Element => {
  const {
    asChild = false,
    className,
    currency = DEFAULT_CURRENCY,
  ...
  return (
    <InputPrimitive
      ...
      {...inputProps}
      className={cn(
        'flex h-9 w-full min-w-0 rounded-md border border-input bg-transparent px-3 py-1 text-base shadow-xs outline-none transition-[color,box-shadow] selection:bg-primary selection:text-primary-foreground file:inline-flex file:h-7 file:border-0 file:bg-transparent file:font-medium file:text-foreground file:text-sm placeholder:text-muted-foreground disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm dark:bg-input/30 border-gray-300',
